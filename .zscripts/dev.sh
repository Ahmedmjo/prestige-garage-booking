...
# CRITICAL: Only run db:push if database file does NOT exist (first run)
# Running db:push on every restart WIPES all data!
if [ ! -f "$PROJECT_DIR/db/custom.db" ]; then
  log_step_start "bun run db:push (first-time setup)"
  echo "[BUN] Creating database for the first time..."
  bun run db:push
  log_step_end "bun run db:push (first-time setup)"
  
  # Auto-seed if seed script exists
  if [ -f "$PROJECT_DIR/scripts/seed.ts" ]; then
    log_step_start "bun run scripts/seed.ts (auto-seed)"
    echo "[BUN] Seeding initial data..."
    bun run scripts/seed.ts 2>&1 | tail -5 || true
    log_step_end "bun run scripts/seed.ts (auto-seed)"
  fi
else
  echo "[BUN] Database already exists — skipping db:push to preserve data"
fi
