...
import { InstallPrompt, ServiceWorkerRegister } from "@/components/pwa/InstallPrompt";
...
export default function Page() {
  ...
  // ⚡ PREEMPTIVE FIRST-VIDEO FETCH — start fetching the first offer's
  // video BYTES during the splash screen so playback starts instantly
  // when HomeScreen mounts.
  const offers = useApp((s) => s.offers);
  useEffect(() => {
    if (!offers || offers.length === 0) return;
    const firstVideoOffer = offers.find(
      (o) => !!(o as { hasVideo?: boolean }).hasVideo,
    );
    if (!firstVideoOffer) return;
    const videoUrl = (firstVideoOffer as { videoUrl?: string | null }).videoUrl;
    if (videoUrl && (videoUrl.startsWith("http") || videoUrl.startsWith("/uploads/"))) {
      return;
    }
    const url = `/api/offers/${firstVideoOffer.id}/video`;
