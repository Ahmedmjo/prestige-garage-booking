...
import { cairoToday } from "@/lib/timezone";
...
// generate next 14 days — uses CAIRO timezone so "today" matches what the
// customer sees on their phone. Without this, a customer booking at 11pm
// Cairo time would see "tomorrow" as "today" because Vercel runs in UTC.
function useDays() {
  return useMemo(() => {
    ...
    // Use Intl to format "today" in Cairo timezone as YYYY-MM-DD.
    // Then iterate forward 14 days by parsing that string and bumping the day.
    const todayStr = cairoToday();
    const [yy, mm, dd] = todayStr.split("-").map(Number);
    // Construct a noon-UTC Date from the Cairo "today" — noon avoids DST midnight drift
    ...
    for (let i = 0; i < 14; i++) {
      ...
      // getDay() returns local-tz day of week — but we want the day-of-week
      // *as seen in Cairo* for the date we just computed. Easiest: use the
      // UTC getters since `key` is the Cairo date.
      const cairoDow = new Date(`${key}T12:00:00Z`).getUTCDay();
      days.push({
        key,
        label: i === 0 ? "اليوم" : i === 1 ? "غداً" : dayNames[cairoDow],
