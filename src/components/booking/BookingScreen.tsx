...
export function BookingScreen() {
  ...
  // ⚡ AUTO-ADVANCE: After selecting a variant, automatically move to the
  // date step after 400ms. No need for the user to press "Next".
  useEffect(() => {
    if (step !== variantStepIdx || !selectedVariant) return;
    const t = setTimeout(() => setStep((s) => s + 1), 400);
    return () => clearTimeout(t);
  }, [step, variantStepIdx, selectedVariant]);

  // ⚡ AUTO-ADVANCE: After selecting both date AND time, automatically move
  // to the details step after 400ms. Wait for slots to load before advancing.
  useEffect(() => {
    if (step !== dateStepIdx || !date || !time || slotsLoading) return;
    const t = setTimeout(() => setStep((s) => s + 1), 400);
    return () => clearTimeout(t);
  }, [step, dateStepIdx, date, time, slotsLoading]);
                ...
                {slots.map((slot) => {
                  ...
                  return (
                    ...
                    >
                      {slot.time}
                      {disabled && (
                        <span className="absolute -top-1.5 -right-1.5 grid h-4 min-w-4 place-items-center rounded-full bg-[#ef4444] px-1 text-[8px] font-bold text-white">
                          مكتمل
                        </span>
                      )}
                      {!active && !disabled && slot.capacity > 0 && slot.capacity > 1 && (
                        <span className="absolute -top-1.5 -right-1.5 grid h-4 min-w-4 place-items-center rounded-full bg-[#25D366] px-1 text-[8px] font-bold text-black">
                          {slot.remaining}
                        </span>
                      )}
                    </button>
