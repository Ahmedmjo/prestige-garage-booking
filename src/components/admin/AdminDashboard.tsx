...
/* ============ BRANCH MAP URL HELPER ============
   Previously, branches stored {map, image, video} as a JSON blob inside the
   mapUrl column. That caused two bugs on the customer side:
     1. <a href="{...}"> hangs the mobile browser (the JSON cannot be parsed as a URL).
     2. A truncated base64 image would corrupt the JSON, breaking the map link too.

   The media feature has been REMOVED entirely per user request. mapUrl now
   always stores a plain map URL string (or null). This helper exists only
   to migrate legacy data: if a branch still has a JSON-packed mapUrl in the
   DB, extract just the .map field and ignore the rest.
*/
function extractMapUrl(mapUrl: string | null | undefined): string {
  if (!mapUrl) return "";
  ...
  if (trimmed.startsWith("{")) {
    try {
      ...
      return typeof obj.map === "string" ? obj.map.trim() : "";
    } catch {
      return "";
    }
  }
...
function BranchesManager({ pin }: { pin: string }) {
  ...
  const save = async (b: BranchItem) => {
    ...
    setSaving(b.id);
    // Media feature was removed — mapUrl is now always a plain string.
    const cleanMapUrl = ed.mapUrl.trim() || null;
              ...
              <div className="p-3.5">
                ...
                {isEditing ? (
                  ...
                  <>
                    ...
                    {extractMapUrl(b.mapUrl) && (
                      <a
                        href={extractMapUrl(b.mapUrl)}
                        target="_blank"
                        rel="noreferrer"
                        title="فتح الخريطة"
                        className="mt-2 inline-flex items-center gap-1 rounded-md border border-[#60a5fa]/30 bg-[#60a5fa]/10 px-2 py-1 text-[10px] font-bold text-[#60a5fa] transition hover:bg-[#60a5fa]/20"
                        dir="ltr"
                      >
                        <ExternalLink size={11} /> فتح الخريطة
                      </a>
                    )}
...
function AddBranchModal({
  ...
  const submit = async () => {
    ...
    setSaving(true);
    // Media feature was removed — mapUrl is now always a plain string.
    const cleanMapUrl = form.mapUrl.trim() || null;
